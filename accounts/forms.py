...
class MyCustomSignupForm(SignupForm):
    ...
    age = forms.IntegerField(label='年齢', required=True, min_value=0, max_value=150,default=0)
    born_at = forms.DateField(label='生年月日', required=True, widget=forms.SelectDateWidget(years=range(1900,2021)))
    introduction = forms.CharField(label='自己紹介', required=False, widget=forms.Textarea(attrs={'placeholder':'はじめまして。太郎です！'}))
